...
router.post('/', authenticate, authorize(UserRole.ADMIN), [
  ...
  try {
    ...
    const { email, firstName, lastName, role, phone, college, year, department } = req.body
    ...
    const defaultPassword = 'Yugam@123'
    const hashedPassword = await bcrypt.hash(defaultPassword, 10)

    // Generate Yugam ID and QR Code
    const { generateYugamId, generateQRCode } = await import('../utils/yugamId')
